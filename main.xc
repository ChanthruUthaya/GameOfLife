...
#define  IMHT 1320                  //image height
#define  IMWD 1320                  //image width
    ...
    for(int y = 1; y <= (IMHT/8); y++){
            ...
            for(int u = 0; u < 3; u++){
                for(int k = 0; k<IMWD/8; k++){
                      for(int i = 0; i<8; i++){
                            bitWorld[u][i+(k*8)] = unPacker(world[(y-1)+u][k],i); //Unpacks all bits and puts it into an array
                          }
                    }
                }

            // computes dead or alive - UNPACKED
            for(int pos = 0; pos < IMWD/8; pos++){
                int result = 0;
                for(int a = 0; a < 8; a++){
                    result = bitPacker(result, (uchar) bitDead(bitWorld, (a + (pos*8)) , bitWorld[1][a + (pos*8)]), a); //Generates new values for board and stores it temporarily in this array.

                }
                worldtemp[y][pos] = (uchar)result;
            }
    }
...
{
  char infname[] = "1320x1320.pgm";     //put your input image path here
  int res; //resolution
